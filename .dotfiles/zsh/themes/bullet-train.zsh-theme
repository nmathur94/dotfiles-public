...
prompt_nvm() {
  ...
  if [[ $(which node | grep -E "(not found)" -c) -eq 1 ]]; then
    return
  fi
  local is_node_project="$(upsearch package.json node_modules)"
  [[ -n "$is_node_project" ]] || return
  local nvm_prompt
...
prompt_php() {
  ...
  if [[ $(which php | grep -E "(not found)" -c) -eq 1 ]]; then
    return
  fi
  local spaces="  "
  # check if php files/project exist
  local is_php_project="$(upsearch composer.json artisan public/index.php)"
  [[ -n "$is_php_project" ]] || return
  
  local php_prompt=$(php -v | grep -oe "^PHP\s*[0-9.]\+" | awk '{print $2}')
  
  prompt_segment $BULLETTRAIN_PHP_BG $BULLETTRAIN_PHP_FG $BULLETTRAIN_PHP_PREFIX$spaces$php_prompt
}
